...
class ChromaDutSearchComponent(LCVectorStoreComponent):
    """Chroma Vector Store filtered by DUT ID."""

    display_name: str = "Chroma DB - DUT Search"
    description: str = "Busca por similaridade no Chroma filtrando por DUT ID"
    name = "ChromaDutSearch"
    icon = "Chroma"
    ...
    inputs = [
        ...
        StrInput(
            ...
            value="/data/vector_database/db",
        ),
        IntInput(
            name="dut_id",
            display_name="DUT ID",
            info="Filtra os casos pelo número da DUT.",
            value=3,
        ),
        *LCVectorStoreComponent.inputs,
        ...
        IntInput(name="limit", display_name="Limit", advanced=True),
        StrInput(
    ...
    @override
    def search_documents(self, vector_store=None) -> Message:
        ...
        metadata_filter = {"dut_id": int(self.dut_id)}
        ...
        if self.search_type == "MMR":
            ...
            records = [
                {"resumo_clinico": doc.page_content, **doc.metadata}
                for doc in docs
            ]
        else:
            ...
            records = [
                {"resumo_clinico": doc.page_content, "score": score, **doc.metadata}
    ...
    def build_vector_store(self) -> Chroma:
        ...
        limit = (
            int(self.limit)
            if self.limit is not None and str(self.limit).strip()
            else None
        )
